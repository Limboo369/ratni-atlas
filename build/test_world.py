...
async def main():
    ...
    async with async_playwright() as p:
        ...
        # a part of the world without states in an era: that era is off there, and the start screen moves to one that has them
        ...
        st = await A.evaluate('() => [window.__ra.ui.settings.map, window.__ra.ui.settings.region, window.__ra.ui.settings.era, document.querySelector("#mapSeg button[data-v=sam]").getAttribute("aria-pressed")]')
        check(st[:2] == ['svijet', 'sam'] and st[2] != 'rim' and st[3] == 'true', f'Sjeverna Amerika in Rim has no states: era moved ({st})')
