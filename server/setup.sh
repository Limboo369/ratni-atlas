...
# smije se pokretati više puta. Pojedine aplikacije objavljuje /usr/local/bin/deovilab-deploy (server/deploy.sh).
set -euo pipefail
export DEBIAN_FRONTEND=noninteractive

apt-get update -qq
apt-get upgrade -y -qq
apt-get install -y -qq nginx certbot ufw fail2ban unattended-upgrades rsync curl ca-certificates
...
# Docker (službeni repozitorij). Objavljeni portovi kontejnera idu samo na 127.0.0.1 ("ip"), jer Docker zaobilazi ufw.
...
# Nginx: zajednička podešavanja + odbijanje nepoznatih domena; ACME izazovi za sve domene idu iz /var/www/letsencrypt
...
# Stara statična verzija igre (prije Dockera); war.deovilab.com sada objavljuje deovilab-deploy
rm -f /etc/nginx/sites-enabled/ratni-atlas /etc/nginx/sites-available/ratni-atlas
rm -rf /var/www/ratni-atlas
